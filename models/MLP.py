...
class MLPSup(nn.Module):
    def __init__(self, args):
        super(MLPSup, self).__init__()
        self.top_bn = args.top_bn
        affine = args.affine
        self.input_len = 1 * 28 * 28
        self.fc1 = nn.Linear(self.input_len, 1200, bias=False)
        self.bn_fc1 = nn.BatchNorm1d(1200, affine=affine)
        self.fc2 = nn.Linear(1200, 600, bias=False)
        self.bn_fc2 = nn.BatchNorm1d(600, affine=affine)
        self.fc3 = nn.Linear(600, 300, bias=False)
        self.bn_fc3 = nn.BatchNorm1d(300, affine=affine)
        self.fc4 = nn.Linear(300, 150, bias=False)
        self.bn_fc4 = nn.BatchNorm1d(150, affine=affine)
        self.fc5 = nn.Linear(150, 10, bias=not self.top_bn)
        if self.top_bn:
            self.bn_fc5 = nn.BatchNorm1d(10, affine=affine)

    def forward(self, x, update_batch_stats=True):
        h = nfunc.relu(call_bn(self.bn_fc1, self.fc1(x.view(-1, self.input_len)), update_batch_stats))
        h = nfunc.relu(call_bn(self.bn_fc2, self.fc2(h), update_batch_stats))
        h = nfunc.relu(call_bn(self.bn_fc3, self.fc3(h), update_batch_stats))
        h = nfunc.relu(call_bn(self.bn_fc4, self.fc4(h), update_batch_stats))
        if self.top_bn:
            h = call_bn(self.bn_fc5, self.fc5(h), update_batch_stats)
        else:
            h = self.fc5(h)
        logits = h
        return logits
