...
class L0VATSym(object):
    ...
    def __call__(self, model, images, log_alpha, l0_ins, kl_way=1):
        ...
        l0_ins.args.eps = 1
        l0_ins.clip = False
        perturbations, train_masks = l0_ins.mask_filter(images, log_alpha)
        sym_perturbations = images * (2 - train_masks)
        ...
        if abs(self.args.lamb2 - 0) > 0.001:
            smooth_loss = l0_ins.smooth_loss(train_masks)
        ...
        lds += self.args.alpha * (compute_lds(model, perturbations, self.dis_criterion, prob_x, log_prob_x, kl_way=kl_way) +
                                  compute_lds(model, sym_perturbations, self.dis_criterion, prob_x, log_prob_x, kl_way=kl_way))

        if 'ent' in self.args.trainer:
            lds += entropy(logits)

        index = random.choice([0, 1])
        return lds, [train_masks, 2 - train_masks][index]
