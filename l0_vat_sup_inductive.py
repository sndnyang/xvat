...
def parse_args():
    parser = argparse.ArgumentParser(description='L0-VAT Inductive Supervised learning in PyTorch')
    ...
    parser.add_argument('--trainer', type=str, default='inl0', help='ce, vat, inl0(L0, not ten), inl0vat (default: inl0)')
    ...
    if args.vis:
        args.dir_path = form_dir_path("L0VAT-sup", args)
...
def train(dataset_kit, model_kit, args):
    ...
    if "l0at" == args.trainer:
        reg_component = IndL0ATOne(args)
    if "at" == args.trainer:
        reg_component = AT(args)
    if "ax" == args.trainer:
        reg_component = AXVAT(args)
    if "axone" == args.trainer:
        reg_component = AXVATOne(args)
    ...
    if args.vis and "l0" in args.trainer and "fig" in args.log_arg:
        ...
        if args.dataset not in ['mnist', 'svhn']:
            ...
            vis_dataset, _ = get_data(args)
            vis_set = vis_dataset.data[idx]
            args.data_dir = temp
        log_alpha = generator(torch.FloatTensor(train_set.data[idx]).to(args.device))
        # show_and_save_generated_demo(l0_ins, log_alpha, train_set.data[idx], args, 0, 0.1, 1, vis_set=vis_set)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        for it in range(args.num_batch_it):
            ...
            if "ce" == args.trainer:
                pass
            elif "l0at" == args.trainer:
                ul_loss, mask = reg_component(model, images, labels, generator, l0_ins, dis='ce' if args.kl == 0 else 'kl')
                total_loss = (1 - args.alpha) * sup_loss + ul_loss
            elif 'at' == args.trainer:
                ul_loss, mask = reg_component(model, images, labels)
                total_loss = (1 - args.alpha) * sup_loss + ul_loss
            elif "inl0" in args.trainer:
                ul_loss, mask = reg_component(model, images, generator, l0_ins, kl_way=args.kl)
                total_loss += ul_loss
                alpha_opt.zero_grad()
            elif "ax" == args.trainer:
                ul_loss, mask = reg_component(model, images, generator, l0_ins, alpha_opt, kl_way=args.kl)
                total_loss += ul_loss
            ...
            # if "inl0" == args.trainer:
            if args.trainer in ["inl0", 'axone'] and (epoch < args.epoch_decay_start or (epoch > args.epoch_decay_start and not decay_stop)):
                # one stage
            ...
            if ((epoch % args.log_interval) == 0 and it == args.num_batch_it - 1) or (args.debug and it < 5 and epoch < 2):
                ...
                if args.vis and it == args.num_batch_it - 1:
                    ...
                    if "l0" in args.trainer and epoch % save_interval == 0 and "fig" in args.log_arg:
                        # log_alpha = generator(torch.FloatTensor(train_set.data[idx]).to(args.device))
                        # show_and_save_generated_demo(l0_ins, log_alpha, train_set.data[idx], args, epoch+1, acc, saved_count, vis_set=vis_set)
                        saved_count += 1
                start_time = time.time()
        ...
        if args.vis:
            ...
            show_perturbations(train_set.data, generator, l0_ins, epoch, args)
